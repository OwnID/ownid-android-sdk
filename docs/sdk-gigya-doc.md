...
The OwnID Gigya-Android SDK integrates with Email/Password-based [Gigya Authentication](https://github.com/SAP/gigya-android-sdk) for apps that use Gigya Core or Gigya Screen-Sets. If your app uses Gigya Screen-Sets only, you should use the OwnID Gigya-Screen-Sets Android SDK instead. For instructions, see the **[Gigya-Screen-Sets](sdk-gigya-screens-doc.md)** guide.
...
```
The OwnID Gigya-Android SDK is built with Android API version 32 and Java 8, and supports the minimum API version 23.
...
For additional configuration options, including logging and OwnID Web App language, see [Advanced Configuration](#advanced-configuration).
...
### Add OwnID UI
You add the Skip Password feature to your application's Registration screen by including the OwnIdButton view. Add the following to your Registration screen's layout file:
...
Also, the `OwnIdRegisterViewModel.register()` function set Gigya's `profile.locale` value to the first locale from [OwnID Web App language](#ownid-web-app-language) list. You can override this behavior by setting required locale in `GigyaRegistrationParameters` like `GigyaRegistrationParameters(mutableMapOf<String, Any>("profile" to """{"locale":"ru"}"""))`.
...
### Add OwnID UI
Similar to the Registration screen, you add Skip Password feature to your application's Login screen by including the OwnIdButton view. Add the following to your Login screen's layout file:
...
In the case of `GigyaError`, the `GigyaException` is available as a `cause`:<br>
`class GigyaException(public val gigyaError: GigyaError, message: String) : OwnIdException(message)`
...
Logging can also be enabled from the configuration file `assets/ownIdGigyaSdkConfig.json` by adding the optional `enable_logging` parameter:
...
You can use the `OwnId.createGigyaInstance` function to create a custom instance. This function reads from a configuration file when creating the instance. If you would prefer to pass the configuration options using a JSON string, see [Custom Instance Using JSON String](#option-2-custom-instance-using-json-string).

<details open>
<summary>Kotlin</summary>

```kotlin
OwnId.createGigyaInstance(
...
```java
OwnIdGigyaFactory.createInstance(
    context: Context, // Android context
    configurationAssetFileName: String = OwnIdGigya.DEFAULT_CONFIGURATION_FILE_NAME, // JSON configuration file    
    gigya: Gigya<out GigyaAccount> = Gigya.getInstance(), // Gigya instance
    instanceName: InstanceName = OwnIdGigya.DEFAULT_INSTANCE_NAME // Instance name
