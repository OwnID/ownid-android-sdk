...
internal abstract class AbstractStepUI<S : AbstractStep>(@LayoutRes private val contentLayoutId: Int) :
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        runCatching {
            val viewModel = ViewModelProvider(requireActivity()).get(OwnIdNativeFlowViewModelInt::class.java)

            @Suppress("UNCHECKED_CAST")
            currentStep = viewModel.ownIdFlowStep.value as S
            currentStep.ownIdNativeFlowData.canceller.setOnCancelListener(onCancelListener)
    ...
    override fun onDestroyView() {
        ...
        if (isInitSuccessful) {
            currentStep.ownIdNativeFlowData.ownIdCore.localeService.unregisterLocaleUpdateListener(this)
            currentStep.ownIdNativeFlowData.canceller.setOnCancelListener(null)
        }
        super.onDestroyView()
    }
    ...
    protected fun getString(ownIdLocaleKey: OwnIdLocaleKey): String =
        currentStep.ownIdNativeFlowData.ownIdCore.localeService.getString(requireContext(), ownIdLocaleKey)
