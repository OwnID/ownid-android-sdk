...
internal class FidoRegisterAuthStep internal constructor(
    ...
    internal object Factory : AbstractStep.Factory {
        ...
        override fun create(stepJson: JSONObject, ownIdNativeFlowData: OwnIdNativeFlowData, onNextStep: (AbstractStep) -> Unit): AbstractStep =
            FidoRegisterAuthStep(ownIdNativeFlowData, onNextStep, createData(stepJson))
    }
    ...
    override fun run(activity: FragmentActivity) {
        ...
        runCatching {
            ...
            val requestJson = createFidoRegisterOptions(
                ownIdNativeFlowData.context, data.rpId, data.rpName, data.userId, data.userName, data.userDisplayName, data.credIds
            )
