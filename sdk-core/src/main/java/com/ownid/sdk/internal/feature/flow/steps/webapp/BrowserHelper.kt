...
internal object BrowserHelper {
    ...
    /**
     * Open [uriString] in native app if present in system and [skipNativeApp] is false, otherwise
     *
     * if default browser is set and supports Custom Tabs, open [uriString] in Custom Tab, otherwise
     *
     * if default browser is set, open [uriString] in it as a standalone browser, otherwise
     *
     * if multiple browsers are available and no default is set, show browser choose dialog
     * and open [uriString] in selected as standalone browser.
     *
     * @param context           Android [Context] to get Intent.
     * @param uriString         address to be open.
    ...
    internal fun launchUri(context: Context, uriString: String, skipNativeApp: Boolean = false) {
        OwnIdInternalLogger.logD(this, "launchUri", "Uri: $uriString, skippNativeApp: $skipNativeApp")
