...
class IdentityPlatform(val baseApiUrl: String) {
    ...
    fun register(name: String, email: String, password: String, callback: Result<User>.() -> Unit) {
        ...
        doPost(baseApiUrl + "register", postJsonData) {
            onSuccess { login(email, password, callback) }
            onFailure { callback(Result.failure(it)) }
        }
    }

    fun register(name: String, email: String, password: String, ownIdData: String, callback: Result<User>.() -> Unit) {
        val postJsonData = JSONObject()
            .put("name", name)
            .put("email", email)
            .put("password", password)
    ...
    fun getProfile(token: String, callback: Result<User>.() -> Unit) {
        doGet(baseApiUrl + "profile", token) {
            mapCatching { response ->
                ...
                User(responseJson.optString("name").ifBlank { "-" }, responseJson.optString("email").ifBlank { "-" }, token)
            }
                .onSuccess { user ->
                    currentUser = user
